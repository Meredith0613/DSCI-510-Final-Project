...
- Designed a complete data product loop: API collection, cleaning, analysis, visualization, and an interactive recommendation feature.
- Made an unstructured-text pipeline transparent and testable rather than relying on opaque modeling.
- Built role-specific market comparisons and translated them into an actionable candidate-facing skill-gap workflow.
- Used environment variables for secret handling and committed a reproducible snapshot for demos that do not require paid API access.
